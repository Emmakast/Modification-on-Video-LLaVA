...
class LanguageBindVideo(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        Returns:
            CLIPOutput: A dataclass containing the model outputs, including attention weights if requested.
        """
        # Use CLIP model's config for some fields (if specified) instead of those of vision & text components.
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
        ...
        vision_attentions = vision_outputs.attentions if output_attentions else None
        text_attentions = text_outputs.attentions if output_attentions else None

        if not return_dict:
            output = (logits_per_image, logits_per_text, text_embeds, image_embeds, text_outputs, vision_outputs)
            if output_attentions:
                output += (text_attentions, vision_attentions)
