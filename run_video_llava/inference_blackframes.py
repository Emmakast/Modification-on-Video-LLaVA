...
def inference_single_video(video_path, inp, model, processor):
    ...
    # Get the shape of a normal video tensor to create black frames of the same shape
    sample_tensor = video_processor(video_path, return_tensors='pt')['pixel_values']
    if isinstance(sample_tensor, list):
        shape = sample_tensor[0].shape
        device = sample_tensor[0].device
    else:
        shape = sample_tensor.shape
        device = sample_tensor.device

    # Create a black tensor of the same shape (all zeros)
    if isinstance(sample_tensor, list):
        tensor = [torch.zeros_like(video).to(model.device, dtype=torch.float16) for video in sample_tensor]
    else:
        tensor = torch.zeros(shape, dtype=torch.float16, device=model.device)

    key = ['video']
    
    print('tensor shape (black frames):', tensor.shape if not isinstance(tensor, list) else [t.shape for t in tensor])
    print(f"{roles[1]}: {inp}")
